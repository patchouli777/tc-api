package storage

import (
	"context"
	"errors"
	"fmt"
	d "main/internal/livestream/domain"

	"github.com/redis/go-redis/v9"
)

// TODO: ttl 10min for livestreams
type cache struct {
	rdb *redis.Client
	// set of livestream ids sorted by viewers
	sorted *sortedIDStore
	// livestream store
	store *livestreamStore
	// map from username (channel) to livestream id
	userMap *userToIdStore
	// set of livestream ids
	ids *idStore
}

func newCache(rdb *redis.Client) *cache {
	sorted := sortedIDStore{rdb: rdb}
	store := livestreamStore{rdb: rdb}
	userMap := userToIdStore{rdb: rdb}
	ids := idStore{rdb: rdb}

	return &cache{rdb: rdb,
		sorted:  &sorted,
		store:   &store,
		userMap: &userMap,
		ids:     &ids}
}

func (r *cache) add(ctx context.Context, ls d.Livestream) error {
	cmds, err := r.rdb.TxPipelined(ctx, func(p redis.Pipeliner) error {
		r.userMap.addTx(ctx, p, ls.UserName, ls.Id)                // nolint:errcheck
		r.sorted.addTx(ctx, p, ls.CategoryLink, ls.Viewers, ls.Id) // nolint:errcheck
		r.store.addTx(ctx, p, ls)                                  // nolint:errcheck
		r.ids.addTx(ctx, p, ls.Id)                                 // nolint:errcheck
		return nil
	})

	if err != nil {
		return fmt.Errorf("pipeline failed: %w", err)
	}

	for i, cmd := range cmds {
		if cmd.Err() != nil {
			return fmt.Errorf("cmd %d failed: %w", i, cmd.Err())
		}
	}

	return nil
}

func (r *cache) get(ctx context.Context, id int) (*d.Livestream, error) {
	ls, err := r.store.get(ctx, id)
	if err != nil {
		if errors.Is(err, redis.Nil) {
			return nil, d.ErrNotFound
		}

		return nil, err
	}

	return ls, nil
}

func (r *cache) list(ctx context.Context, category string, page, count int) ([]d.Livestream, error) {
	ids, err := r.sorted.get(ctx, category, page, count)
	if err != nil {
		return nil, err
	}

	res, err := r.store.list(ctx, ids)
	if err != nil {
		return nil, err
	}

	return res, nil
}

func (r *cache) update(ctx context.Context, lsId int, title string, u d.User, c d.Category) (*d.Livestream, error) {
	cmds, err := r.rdb.TxPipelined(ctx, func(p redis.Pipeliner) error {
		r.userMap.addTx(ctx, p, u.Name, lsId)
		r.store.updateFieldTx(ctx, p, lsId, map[string]any{
			"user":     u,
			"category": c,
			"title":    title})
		r.ids.addTx(ctx, p, lsId).Err()
		return nil
	})

	if err != nil {
		return nil, fmt.Errorf("pipeline failed: %w", err)
	}

	for i, cmd := range cmds {
		if cmd.Err() != nil {
			return nil, fmt.Errorf("cmd %d failed: %w", i, cmd.Err())
		}
	}

	livestream, err := r.store.get(ctx, lsId)
	if err != nil {
		return nil, err
	}

	return livestream, nil
}

func (r *cache) updateThumbnail(ctx context.Context, id int, thumbnail string) error {
	return r.store.updateThumbnail(ctx, id, thumbnail)
}

func (r *cache) updateViewers(ctx context.Context, id int, viewers int) error {
	ls, err := r.store.get(ctx, id)
	if err != nil {
		return err
	}

	cmds, err := r.rdb.TxPipelined(ctx, func(p redis.Pipeliner) error {
		r.sorted.addTx(ctx, p, ls.CategoryLink, viewers, id)
		r.store.updateViewers(ctx, id, int(viewers))
		return nil
	})

	if err != nil {
		return fmt.Errorf("pipeline failed: %w", err)
	}

	for i, cmd := range cmds {
		if cmd.Err() != nil {
			return fmt.Errorf("cmd %d failed: %w", i, cmd.Err())
		}
	}

	return nil
}

func (r *cache) delete(ctx context.Context, id int) error {
	ls, err := r.store.get(ctx, id)
	if err != nil {
		return err
	}

	if ls == nil {
		return d.ErrAlreadyEnded
	}

	cmds, err := r.rdb.TxPipelined(ctx, func(p redis.Pipeliner) error {
		r.userMap.deleteTx(ctx, p, ls.UserName)
		r.sorted.deleteTx(ctx, p, ls.CategoryLink)
		r.store.deleteTx(ctx, p, id)
		r.ids.deleteTx(ctx, p, id)
		return nil
	})

	if err != nil {
		return fmt.Errorf("pipeline failed: %w", err)
	}

	for i, cmd := range cmds {
		if cmd.Err() != nil {
			return fmt.Errorf("cmd %d failed: %w", i, cmd.Err())
		}
	}

	return nil
}

func (r *cache) existsByUsername(ctx context.Context, username string) (bool, error) {
	id, err := r.userMap.get(ctx, username)
	if err != nil || id == -1 {
		return false, err
	}

	return true, nil
}
